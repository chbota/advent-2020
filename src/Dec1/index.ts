...
export function findSum(nums: number[], sumToFind: number, numToSum: number): number {
	...
	const matchesDesiredSum = () => {
		return candidateIndices.reduce((accum, current) => accum + nums[current], 0) === sumToFind;
	}
	...
	while (!done) {
		if (!shouldSkipCandidates() && matchesDesiredSum()) {
			return candidateIndices.reduce((accum, current) => accum * nums[current], 1);
		}
...
export function loadData(): number[] {
	const input = fs.readFileSync(path.join(process.cwd(), 'src/Dec1/input.txt'), { encoding: 'utf-8'});
