...
function updateSearchLocation(startingLocation: SearchLocation, routeEntry: string): SearchLocation {
    const colMid = getMidpoint(startingLocation.colStart, startingLocation.colEnd);
    const rowMid = getMidpoint(startingLocation.rowStart, startingLocation.rowEnd);
...
export function findSeat(numColumns: number, numRows: number, seatRoute: string): Seat {
    ...
    if (currentSearchLocation.colStart !== currentSearchLocation.colEnd) {
        throw new Error(`Didnt finish column search: ${currentSearchLocation.colStart}, ${currentSearchLocation.colEnd}`);
    }

    if (currentSearchLocation.rowStart !== currentSearchLocation.rowEnd) {
        throw new Error(`Didnt finish row search: ${currentSearchLocation.rowStart}, ${currentSearchLocation.rowEnd}`);
    }

    return { row: currentSearchLocation.rowStart, column: currentSearchLocation.colStart };
}
